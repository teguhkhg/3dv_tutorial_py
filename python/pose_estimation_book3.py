    ...
    while True:
        ...
        try:
            H, inlier_mask = cv2.findHomography(
                np.asarray(img_points, dtype=np.float32), np.asarray(obj_project, dtype=np.float32), cv2.RANSAC, 2)
        except:
            continue
        ...
        if inlier_num > min_inlier_num:
            ...
            for i in range(len(inlier_mask)):
                if inlier_mask[i][0]:
                    obj_inlier.append(obj_points[i])
                    img_inlier.append(img_points[i])
        ...
        info = "Inliers: %d (%d%%), Focal Length: %.0f" % (
            inlier_num, 100*inlier_num/len(matches), K[0, 0])
